...
% Mean intensity percentage vector
MAXINTEN = 0;
meanIntensityPtg = [];
...
meanIntensityPtg = meanIntensityPtg ./ double(MAXINTEN) .* 100;
[~, L] = size(area);
for i = 1 : L
    featureMap(end+1,:) = [area(i), mdc(i), meanIntensityPtg(i)];
end
